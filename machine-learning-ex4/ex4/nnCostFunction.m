...
% Transform the vector y to a tow dimension matrix

label_size = size(Theta2, 1);
yMatrix = zeros(m, label_size);
for i = 1:m
    yMatrix(i, y(i)) = 1;
end
...
%for i = 1:m
%    J = J + sum(sum( -( yMatrix(i,:) .* log( h(i,:) ) ) - ( ( 1 - yMatrix(i,:) ) .* log( 1 - h(i,:) ) ) ));
%end
%J = ( 1 / m ) * J;

% Add the regularizeda part

t1 = sum(sum(Theta1(:,2:end).^2));
t2 = sum(sum(Theta2(:,2:end).^2));
J = J + (lambda/(2*m)) * (t1 + t2);

% Start generate the grad
